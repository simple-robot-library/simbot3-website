...
<tbody>
    <tr><td>code</td><td>账号</td></tr>
    <tr><td>password</td><td>明文密码，与下面的 passwordMD5二选一。</td></tr>
    <tr><td>passwordMD5</td><td>MD5加密后的密码，与上面的 password 二选一。</td></tr>
    <tr><td>deviceInfoSeed</td><td>mirai配置自定义deviceInfoSeed的时候使用的随机种子。默认为1</td></tr>
    <tr><td>workingDir</td><td>同原生mirai配置，mirai的工作目录。默认为 .</td></tr>
    <tr><td>heartbeatPeriodMillis</td><td>同原生mirai配置，连接心跳包周期</td></tr>
    <tr><td>statHeartbeatPeriodMillis</td><td>同原生mirai配置，状态心跳包周期</td></tr>
    <tr><td>heartbeatTimeoutMillis</td><td>同原生mirai配置，每次心跳时等待结果的时间</td></tr>
    <tr>
        <td>heartbeatStrategy</td>
        <td>
    ...
    <tr><td>reconnectionRetryTimes</td><td>同原生mirai配置，最多尝试多少次重连   </td></tr>
    <tr><td>autoReconnectOnForceOffline</td><td>同原生mirai配置，Boolean类型。在被挤下线时 (BotOfflineEvent.Force) 自动重连。</td></tr>
    <tr>
        <td>protocol</td>
        <td>
    ...
    <tr><td>highwayUploadCoroutineCount</td><td>同原生mirai配置，Highway 通道上传图片, 语音, 文件等资源时的协程数量。</td></tr>
    <tr><td>deviceInfo</td><td>使用的自定义设备信息。详见下文。</td></tr>
    <tr><td>simpleDeviceInfo</td><td>使用的自定义设备信息的简化可读版。详见下文。</td></tr>
    <tr><td>deviceInfoFile</td><td>指定设备信息文件。详见下文。</td></tr>
    <tr><td>noNetworkLog</td><td>不展示mirai网络日志。默认false</td></tr>
    <tr><td>noBotLog</td><td>不展示mirai Bot日志。默认false</td></tr>
    <tr><td>isShowingVerboseEventLog</td><td>同原生mirai配置，是否显示过于冗长的事件日志。默认false</td></tr>
    <tr><td>cacheDir</td><td>同原生mirai配置，缓存数据目录, 相对于 workingDir</td></tr>
    <tr><td>contactListCache</td><td>同原生mirai配置，Json Object 格式。详见下文。</td></tr>
    <tr>
        <td>loginCacheEnabled</td>
        <td>同原生mirai配置，登录缓存。开启后在密码登录成功时会保存秘钥等信息, 在下次启动时通过这些信息登录, 而不提交密码. 可以减少验证码出现的频率. <br />
    秘钥信息会由密码加密保存. 如果秘钥过期, 则会进行普通密码登录。默认为true。</td>
    </tr>
    <tr>
        <td>convertLineSeparator</td>
        <td>
